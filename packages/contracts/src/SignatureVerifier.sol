...
library SignatureVerifier {
     ...
     * @param response: An ABI encoded tuple of
     * `(bytes result, uint64 expires, bytes sig)`, where `result` is the data to return
        ...
        returns (address, bytes memory)
    {
        ...
        (bytes memory extraData, address sender) =
            abi.decode(request, (bytes, address));
        address signer = ECDSA.recover(
            makeSignatureHash(sender, expires, extraData, result), sig
        );
