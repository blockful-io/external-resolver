...
export class ArbProofService<chain extends Chain> implements IProofService<ArbProvableBlock> {
  ...
  async getStorageAt(block: ArbProvableBlock, address: Address, slot: bigint): Promise<Hash> {
    return this.helper.getStorageAt(BigInt(block.number), address, slot);
  }
   ...
   */
  async getProofs(block: ArbProvableBlock, address: Address, slots: bigint[]): Promise<Hash> {
    const proof = await this.helper.getProofs(BigInt(block.number), address, slots);
  ...
  private async getBlockHashAndSendRoot(nodeNum: bigint): Promise<[Hash, Hash]> {
    ...
    const allLogs = await this.l1Provider.getLogs({
      address: this.rollupAddress as Hash,
      fromBlock,
      event: parseAbiItem(
        'event NodeCreated(uint64 indexed nodeNum, bytes32, bytes32, bytes32, (((bytes32[2],uint64[2]), uint8),((bytes32[2],uint64[2]), uint8), uint64), bytes32, bytes32, uint256)'
      )
    });

    // had to manually filter because the TS type inference of this function seems broken
    const logs = allLogs.filter((log) => log.args.length > 0 && log.args[0] === nodeNum).reverse();

    if (logs.length === 0) {
      throw new Error('No logs found for nodeNum ' + nodeNum);
    }
    ...
    const blockHash = '0x' + logs[0].data.substring(blockHashCharIndex, blockHashCharIndex + 64);
    const sendRoot = '0x' + logs[0].data.substring(sendRootCharIndex, sendRootCharIndex + 64);
